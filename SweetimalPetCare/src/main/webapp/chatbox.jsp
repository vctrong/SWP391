...
(function(){
    ...
    // Fallback replies for deep actions in case server returns default greeting
    const deepFallback = {
    ...
    // Deeper options for each primary action
    const deepOptions = {
        emergency: [
    ...
    function renderMainOptions(){
        // Create a fresh options container and append at bottom
    ...
    function renderBackButton(){
        ...
        // Link displayed below the back button
    ...
    function clearDeepBlocks(){
        // Remove all deep option blocks and back button blocks
    ...
    if(minimizeBtn){
        minimizeBtn.addEventListener('click', function(){
            // Reset chat state: clear messages, reset flags, show greeting and main options
            chosen = false;
            ...
            // Render main options fresh
            renderMainOptions();
            // Ensure chat window stays open (do not toggle/close)
    ...
    function appendBotMessage(text){
        ...
        // Use textContent to avoid HTML injection, and let CSS handle newlines
    ...
    function sendAction(action, label){
        ...
        // After first level choose, remove the initial options entirely;
        // defer rendering deeper options until after bot reply so they appear below it
        let optionsContainer = getOptionsContainer();
        if (optionsContainer && !chosen) {
            chosen = true;
            // Remove the initial options block so nothing shows above the doctor reply
        ...
        fetch(endpoint, {
            ...
            .then(function(data){
                // Only show bot reply for the selected option, no greeting or deep chat link
                ...
                if ((action||'').indexOf('_') !== -1 && (msg||'').trim() === defaultGreeting) {
                    // Server replied with default greeting unexpectedly; use client fallback
                    if (deepFallback[action]) {
                        msg = deepFallback[action];
                    }
                }
                appendBotMessage(msg);

                // After bot reply for first-level action, show deeper options below this reply
                ...
                // For deep selections, remove existing deep option buttons after reply and only show the back button
            ...
            .catch(function(){ appendBotMessage('Xin lỗi, đã có lỗi kết nối.'); });
    }
    ...
    function onOptionClick(e){
        ...
        if(action === 'back_main'){
            // Reset state and show main options below last reply
            chosen = false;
            pendingDeepBase = null;
            clearDeepBlocks();
            renderMainOptions();
